...
public class UuidManager {
  ...
  public void setTemplate(JdbcTemplate m) {
    template = m;
  }
  ...
  public UUID getUuid(String entityId) {
    ...
    // the above should hit most of the time...for InCommon SPs it will take some extra work
      ...
      if (uuidIc.size() > 0) {
        ...
        // we have never done anything in the database with this SP, so we have to create a UUID for
        // it
        UUID newUuid = UUID.randomUUID();
        template.update(
            "insert into incommon_sp (uuid, entity_id, active) values " + "(? ,?, true)",
            newUuid,
            entityId);

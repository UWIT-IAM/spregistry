...
public class RelyingParty implements XMLSerializable {
    ...
    for (int i = 0; i < nl1.getLength(); i++) {
      ...
      if (XMLHelper.matches(name, "SPSSODescriptor")) {
        ...
        for (int j = 0; j < nl2.getLength(); j++) {
          ...
          if (XMLHelper.matches(name2, "AssertionConsumerService"))
            assertionConsumerServices.add(new AssertionConsumerService(e2));
          if (XMLHelper.matches(name2, "ManageNameIDService"))
            manageNameIDServices.add(new ManageNameIDService(e2));
        }
      }
  ...
  public RelyingParty(String id, String dns) {
    ...
    assertionConsumerServices.add(
        new AssertionConsumerService(
            1,
            "urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST",
            "https://" + dns + "/Shibboleth.sso/SAML2/POST"));
  ...
  public void writeXml(BufferedWriter xout) throws IOException {
    ...
    xout.write(
        "  <SPSSODescriptor "
            ...
            + XMLHelper.safeXml(protocolSupportEnumerationsUnsplit)
            + "\">\n");
    ...
    for (int i = 0; i < nameIDFormats.size(); i++) {
      xout.write(
          "   <NameIDFormat>" + XMLHelper.safeXml(nameIDFormats.get(i)) + "</NameIDFormat>\n");
    }

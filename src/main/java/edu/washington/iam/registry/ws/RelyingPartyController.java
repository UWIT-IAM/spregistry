...
public class RelyingPartyController {
  ...
  private String googleIdentityProvider =
      "https://idp-gw.u.washington.edu/google"; // value overridden by spreg.properties
  ...
  private ModelAndView loginChooserMV(
      RPSession session, HttpServletRequest request, HttpServletResponse response) {
  ...
  private RPSession processRequestInfo(
      ...
      if (certs != null) {
        ...
        try {
          ...
          if (altNames != null) {
            ...
            session.altNames.add(
                session.remoteUser); // rules say cn meaningful only when altnames not present
  ...
  private ModelAndView loginPage(
    ...
    // String remoteUser = "mattjm@washington.edu";  //netid@washington.edu goes here for testing
    // locally
  ...
  public ModelAndView getRelyingParties(
      @RequestParam(value = "selectrp", required = false) String selRp,
    ...
    List<RelyingParty> relyingPartyIds =
        rpManager.getRelyingParties(selRp, selMine == null ? null : session.remoteUser);
  ...
  public ModelAndView getRelyingParty(
  ...
  public ModelAndView getRelyingPartyNew(
      @RequestParam(value = "rpid", required = true) String rpid,
      @RequestParam(value = "mdid", required = false) String mdid,
      @RequestParam(value = "view", required = false) String view,
      @RequestParam(value = "role", required = false) String role,
      @RequestParam(value = "lookup", required = false)
          String lookup, // enter manual data for this entity
      HttpServletRequest request,
      HttpServletResponse response) {
    ...
    if (lookup != null) {
      ...
      try {
        if (lookup.equals("sp"))
          lookup = "https://" + hostPortFromEntityId(rpid) + "/Shibboleth.sso/Metadata";
        ...
        if (!hostPortFromEntityId(rpid).equals(hostPortFromEntityId(rp.getEntityId()))) {
          log.info(
              String.format(
                  "requested dns '%s' not equal to fetched entityId '%s'",
                  hostPortFromEntityId(rpid), rp.getEntityId()));
          return emptyMV(
              String.format(
                  "The requested entity id's domain name, '%s', does not match the fetched name: '%s'",
                  hostPortFromEntityId(rpid), rp.getEntityId()));
        }
  ...
  public ModelAndView putRelyingParty(
      @RequestParam(value = "id", required = true) String id,
      @RequestParam(value = "mdid", required = true) String mdid,
      @RequestParam(value = "role", required = false) String role,
    ...
    try {
      ...
      relyingParty =
          new RelyingParty(doc.getDocumentElement(), mdid, rpManager.isMetadataEditable(mdid));
    } catch (Exception e) {
  ...
  public ModelAndView deleteRelyingParty(
      @RequestParam(value = "id", required = true) String id,
      @RequestParam(value = "mdid", required = true) String mdid,
      @RequestParam(value = "role", required = false) String role,
      @RequestParam(value = "xsrf", required = false) String paramXsrf,
    ...
    try {
      ...
      mv.addObject(
          "alert",
          "(while attempting  rp delete) delete of filter policy failed:\n" + e.getCause());
      response.setStatus(500);
      return mv;
    } catch (DNSVerifyException e) {
      mv.addObject(
          "alert", "(while attempting  rp delete) Could not verify ownership:\n" + e.getCause());
      response.setStatus(500);
      return mv;
    } catch (ProxyException e) {
      mv.addObject(
          "alert", "(while attempting  rp delete) Proxy (gateway) error:\n" + e.getCause());
      response.setStatus(500);
      return mv;
  ...
  public ModelAndView getRelyingPartyAttributes(
      @RequestParam(value = "id", required = true) String id,
      @RequestParam(value = "mdid", required = false) String mdid,
      @RequestParam(value = "view", required = false) String view,
      HttpServletRequest request,
  ...
  public ModelAndView putRelyingPartyAttributes(
      @RequestParam(value = "id", required = true) String id,
  ...
  public ModelAndView putRelyingPartyAttrReq(
      @RequestParam(value = "id", required = true) String id,
      InputStream in,
      HttpServletRequest request,
      HttpServletResponse response) {
  ...
  public ModelAndView putRelyingPartyAttributesZ(
      @RequestParam(value = "id", required = true) String id,
      @RequestParam(value = "role", required = false) String role,
      @RequestParam(value = "xsrf", required = false) String paramXsrf,
  ...
  public ModelAndView putRelyingPartyAccessCtrl(
      @RequestParam(value = "id", required = true) String id,
      @RequestParam(value = "role", required = false) String role,
      @RequestParam(value = "xsrf", required = false) String paramXsrf,
  ...
  public ModelAndView putAccessCtrlReq(
      @RequestParam(value = "id", required = true) String id,
      InputStream in,
      HttpServletRequest request,
      HttpServletResponse response) {
    ...
    if (doc != null) {
      ...
      for (int i = 0; i < attrs.size(); i++)
        txt.append("  Add new access control feature: " + attrs.get(i).getAttribute("id") + "\n\n");
      attrs = XMLHelper.getElementsByName(doc.getDocumentElement(), "Drop");
      log.debug(attrs.size() + " access control drops");
      for (int i = 0; i < attrs.size(); i++)
        txt.append(
            "  Drop existing access control feature: " + attrs.get(i).getAttribute("id") + "\n\n");
      Element mele = XMLHelper.getElementByName(doc.getDocumentElement(), "Comments");
      if (mele != null) txt.append("\nComment:\n\n" + mele.getTextContent() + "\n\n");
      txt.append("Quick link:\n\n   " + spRegistryUrl + "#a" + id + "\n");
  ...
  public void setUuidManager(UuidManager m) {
    uuidManager = m;
  }

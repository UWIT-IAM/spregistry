...
public class SRConnectionManager {
   ...
   private static Logger log = LoggerFactory.getLogger(SRConnectionManager.class);
   
   public SRConnectionManager(String baseurl, String caFile, String certFile, String keyFile) {
      ...
      try {
         ...
         if (port<0) {
            ...
            log.info("gws port not specified, using " + port);
         }
      ...
      try {
         ...
         log.debug("ctx init {" + ctx + "} = " + keyManagers.length + " : " + trustManagers.length);
         ctx.init(keyManagers, trustManagers, null);
         socketFactory = new SSLSocketFactory(ctx);
         Scheme scheme = new Scheme(url.getProtocol(), socketFactory, port);

...
public class AccessCtrlManagerDBTest {
  ...
  @Autowired private AccessCtrlManagerDB dao;
  ...
  public void testInitialAccessCtrl() {
    ...
    template.update("delete from access_control where entity_id = ?", fakeEntityIds.get(0));
  ...
  public void testEnableStuff() {
    ...
    List<Integer> ids =
        template.queryForList(
            "select id from access_control where entity_id = ?", Integer.class, entityId);
    // should just be 1
  ...
  public void testDisableAllStuff() {
    ...
    try {
      ...
      log.info("test delete access control threw exception");
    }

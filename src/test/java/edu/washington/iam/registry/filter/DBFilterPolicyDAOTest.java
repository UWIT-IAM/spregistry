...
public class DBFilterPolicyDAOTest {
  ...
  @Autowired private DBFilterPolicyDAO dao;

  @Autowired private JdbcTemplate template;

  private String remoteUser = "mattjm";

  private List<String> fakeEntityIds =
      Arrays.asList(
  ...
  public void testGetFilterPolicies() throws Exception {
    ...
    for (String fakeId : fakeEntityIds) {
      template.update(
          "insert into filter (uuid, group_id, entity_id, xml, end_time, start_time) "
              + "values (?, ?, ?, ?, null, now())",
          genUUID(),
          filterPolicyGroup.getId(),
          fakeId,
          fakeAttributeFilterPolicyXml(fakeId));
    }
    ...
    namedTemplate.update(
        "delete from filter where entity_id in (:entityIds)",
  ...
  public void testAttributeFilterPolicyFromElement() throws Exception {
    ...
    FilterPolicyGroup filterPolicyGroup = new FilterPolicyGroup();
    filterPolicyGroup.setId("uwrp");
    AttributeFilterPolicy afp =
        dao.attributeFilterPolicyFromElement(
    ...
    Assert.assertEquals(
        inFilterPolicyXml.replaceAll("\\s+", ""), sw.toString().replaceAll("\\s+", ""));
  }

  @Test
  public void testAttributeFilterPolicyParseComplexRegex() throws Exception {
    String inFilterPolicyXml =
    ...
    Element afpElement =
        DocumentBuilderFactory.newInstance()
            ...
            .parse(new ByteArrayInputStream(inFilterPolicyXml.getBytes()))
            .getDocumentElement();
    AttributeFilterPolicy afp = dao.attributeFilterPolicyFromElement(afpElement, filterPolicyGroup);
  ...
  @Test
  @Ignore(
      "The possibility of a null AttributeFilterPolicy is understood, consider removing this test")
  ...
  public void testCreateFilterPolicy() throws Exception {
    ...
    AttributeFilterPolicy attributeFilterPolicy =
        fakeAttributeFilterPolicy(filterPolicyGroup, entityId);

    template.update("delete from filter where entity_id = ?", entityId);
    List<Timestamp> qResults =
        template.queryForList(
            "select entity_id from filter where entity_id = ?",
            new Object[] {entityId},
            Timestamp.class);
    ...
    qResults =
        template.queryForList(
            "select start_time from filter where entity_id = ?",
            new Object[] {entityId},
            Timestamp.class);
  ...
  public void testCreateFilterPolicyPreviouslyDeleted() throws Exception {
    ...
    template.update("delete from filter where entity_id = ?", entityId);
    template.update(
        "insert into filter (uuid, group_id, entity_id, xml, end_time, start_time) values (?, ?, ?, ?, now(), now())",
        genUUID(),
        filterPolicyGroup.getId(),
        entityId,
    ...
    List<Timestamp> qResults =
        template.queryForList(
            "select start_time from filter where entity_id = ? and end_time is null",
            new Object[] {entityId},
            Timestamp.class);
    Assert.assertEquals(0, qResults.size());
    Timestamp preUpdateTime = new Timestamp(new Date().getTime());
  ...
  public void testUpdateFilterPolicy() throws Exception {
    ...
    template.update("delete from filter where entity_id = ?", entityId);
    template.update(
        "insert into filter (group_id, entity_id, xml, start_time, end_time, uuid) values (?, ?, ?, now(), ?, ?)",
        filterPolicyGroup.getId(),
        entityId,
        fakeAttributeFilterPolicyXml(entityId),
        null,
  ...
  public void testUpdateFilterPolicies() throws Exception {
    ...
    FilterPolicyGroup filterPolicyGroup = fakeFilterPolicyGroup();
    namedTemplate.update(
        "delete from filter where entity_id in (:ids)",
    ...
    List<Timestamp> qResults =
        namedTemplate.queryForList(
            "select start_time from filter where entity_id in (:ids)",
            new MapSqlParameterSource().addValue("ids", entityIds),
            Timestamp.class);
  ...
  private void insertFakeData(FilterPolicyGroup filterPolicyGroup, String entityId)
      throws Exception {
    List<UUID> uuid =
        template.queryForList(
            ...
            entityId);
    template.update(
        "insert into filter (uuid, group_id, entity_id, xml, end_time, start_time) values (?, ?, ?, ?, null, now())",
        uuid.get(0),
        filterPolicyGroup.getId(),
        entityId,
        fakeAttributeFilterPolicyXml(entityId));
  ...
  private AttributeFilterPolicy fakeAttributeFilterPolicy(
      FilterPolicyGroup filterPolicyGroup, String entityId) throws Exception {
  ...
  private void setupWithRPs(List<String> entityIds) {
    ...
    for (String entityId : entityIds) {
      ...
      template.update(
          "insert into metadata (uuid, group_id, entity_id, xml, end_time, start_time) "
              + "values (?, ?, ?, ?, ?, now())",
          genUUID(),
          groupId,
          entityId,
          fakeRelyingPartyXml(entityId),
          null);
    }
  ...
  private void teardownRPs(List<String> entityIds) {
    NamedParameterJdbcTemplate namedTemplate = new NamedParameterJdbcTemplate(template);
    namedTemplate.update(
        "delete from metadata where entity_id in (:ids)",
        new MapSqlParameterSource().addValue("ids", entityIds));
  }

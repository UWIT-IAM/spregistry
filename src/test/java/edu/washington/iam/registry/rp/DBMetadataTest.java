...
public class DBMetadataTest {
  @Autowired private DBMetadata dao;
  ...
  public void testSearchRelyingPartyIdsNoDeleted() throws Exception {
    template.update(
        "update metadata set end_time = '2001-01-01' where entity_id = ?",
        "https://searchdbmetadatatest2.s.uw.edu");
  ...
  public void testGetRelyingPartyById() throws Exception {
    RelyingParty relyingParty =
        dao.getRelyingPartyById("https://dbmetadatatest.s.uw.edu/shibboleth");
  ...
  public void testUpdateRelyingPartyNewRP() throws Exception {
    ...
    Assert.assertEquals(fakeEntityIds.size() + 1, ids.size());
    Assert.assertTrue(
        String.format("list of entity ids contains %s", entityId), ids.contains(entityId));
  }
  ...
  public void testUpdateRelyingPartyExistingRP() throws Exception {
    ...
    Assert.assertTrue(
        String.format("update time for %s has changed", fakeEntityIds.get(0)),
  ...
  public void testUpdateRelyingPartyDeletedRP() throws Exception {
    template.update(
        "update metadata set end_time = '2001-01-01' where entity_id = ? ", fakeEntityIds.get(0));
    Timestamp preUpdateTime = new Timestamp(new Date().getTime());
    Thread.sleep(500);
    int preUpdateSize = dao.searchRelyingPartyIds(null).size();
    ...
    // TODO:  Then get the UUID of the entity again here and compare it the previous one--it should
    // be different
  ...
  private Timestamp getTimestampForRP(String entityId) {
    return template.queryForObject(
        "select start_time from metadata where entity_id = ? and end_time is null",
        Timestamp.class,
        entityId);
  }
